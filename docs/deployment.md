...
Ожидаемый результат: 53/53 operations, 31/31 schemas, все обязательные тесты зелёные, live-тесты проходят при переданных синтетических `E2E_*` переменных.
...
Функции закреплены в `arn1` (Stockholm). Этот регион проверен реальным login-flow до backend `45.141.100.245:8080`; прежний `fra1` стабильно завершал BFF-запросы по 10-секундному `TimeoutError`, хотя backend отвечал напрямую. Изменение региона требует preview login-smoke для всех трёх ролей до production deployment.
...
- изменение Vercel compute-region требует preview-проверки прямой связности с backend и login всех ролей;
